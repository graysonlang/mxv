...
- Use `npm run serve` for agent local browser checks; ESP serves on port `8000` by default and does not launch Chrome.
- Use `npm run serve -- --port=8080` with `npm run verify:webgpu` for agent WebGPU checks so they do not fight with a VS Code-launched server on port `8000`.
