...
`npm run verify:naga-wgsl` browser-compile-checks those Naga WGSL outputs with Chrome/WebGPU. The current result is halfway promising: all six generated vertex modules compile in the browser, while all six generated pixel modules fail the same WGSL uniformity rule because `fwidth` is reachable through `mx_subsurface_scattering_approx` under control flow that Chrome treats as possibly non-uniform. `npm run verify:naga-wgsl -- --allow-known-failures` keeps this probe runnable while that blocker is being investigated.
