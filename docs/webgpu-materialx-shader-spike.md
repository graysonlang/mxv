...
`npm run verify:naga-wgsl` browser-compile-checks those Naga WGSL outputs with Chrome/WebGPU. With the current pre-passes, all generated vertex and pixel modules compile as browser WGSL shader modules. This still leaves pipeline layout, resource binding compatibility, and rendered fidelity unproven; the subsurface-radius pre-pass is a deliberate spike compromise, not a claim that the translated shader is visually equivalent to the desktop viewer.
